...
private extension QuestionViewController {
    ...
    func updateUI() {
        ...
        // Получучение текущего вопроса
        let currentQuestion = questions[questionIndex]
        
        // Устанавливаем текущий вопрос для метки вопроса
        questionLabel.text = currentQuestion.title
        
        // Рассчитываем уровень прогресса
        let totalProgress = Float(questionIndex) / Float(questions.count)
        
        // Устанавливаем прогресс для просмотра прогресса
        questionProgressView.setProgress(totalProgress, animated: true)
        
        // Устанавливаем заголовок навигации
        title = "Вопрос № \(questionIndex + 1) из \(questions.count)"
        
        // Показываем варианты ответа, соответствующего номеру(типу) вопроса
        showCurrentAnswers(for: currentQuestion.type)
    }
    
    // ResponseType - отдельная модель данных(перечесление)
